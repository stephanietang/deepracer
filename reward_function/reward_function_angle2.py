 # my version to customize on top https://medium.com/twodigits/aws-deepracer-how-to-train-a-model-in-15-minutes-3a0dca1175fb
# here is the rational, when it needs to take turns on several sharp turns then use the author's original function: namely best steering angle
# if on the relative smoother turns or straight line, then use progress and speed to incentivize the reward
import math
...
def score_encourage_progress_speed(params):
    all_wheels_on_track = params['all_wheels_on_track']
    speed = abs(params['speed'])
    steps = params['steps']
    progress = params["progress"]

    # check the fatest time on the leaderboard
    TARGET_TIME = 15
    TARGET_STEP = TARGET_TIME * 15
    TARGET_CURRENT_STEP = progress / 100 * TARGET_STEP

    if all_wheels_on_track and steps > 0:
        reward = 1
        # change MAX_SPEED according to the action space
        MAX_SPEED = 4

        # reward with high speed
        reward *= speed/MAX_SPEED
        
        # the more steps used, the reward is lower      
        reward *= (TARGET_CURRENT_STEP / steps)**2
...
def reward_function(params):
    ...
    # there are several sharp turns
    if closest_waypoints > 55 and closest_waypoints < 98:
        return float(score_steer_to_point_ahead(params))
    
    # use encourage progress and speed if less sharp turns
    return score_encourage_progress_speed(params)
...
def test_score_steer_to_point_ahead():
    ...
    print("Test optimized steering angle: Scores: {}, {}, {}, {}, {}".format(sc(params_l_45), sc(params_l_15), sc(params_0), sc(params_r_15), sc(params_r_45)))

def test_score_encourage_progress_speed():
    params1 = {**get_test_params(), 'speed': 4, 'steps': 225}
    params2 = {**get_test_params(), 'speed': 4, 'steps': 250}
    params3 = {**get_test_params(), 'speed': 2.5, 'steps': 225}
    params4 = {**get_test_params(), 'speed': 3, 'steps': 225}
    params5 = {**get_test_params(), 'speed': 3, 'steps': 250}

    sc = score_encourage_progress_speed
    print("Encourage progress and speed: Scores: {}, {}, {}, {}, {}".format(sc(params1), sc(params2), sc(params3), sc(params4), sc(params5)))
...
def run_tests():
    ...
    test_score_encourage_progress_speed()
