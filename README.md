...
- aug12, use reward function as aug08 but use a different action space with faster speed model_metadata_19_linear_AS_1.5_4.json, train for 3h
- aug13, use aug2024_arctic_open_ccw_13 but use a different action space with faster speed model_metadata_19_linear_AS_1.5_4.json, train for 3h, to compare with aug12
...
aws ec2 describe-instances --filters "Name=instance-state-name,Values=running" --output json | jq '.Reservations[].Instances[].InstanceId'
